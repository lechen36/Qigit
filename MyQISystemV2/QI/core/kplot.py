...
def kplot(df,index):
    ...
    if index=='MACD':
        ...
        df_vol1=df[['iNum','vol','si']]
        df_vol1=df_vol1[df_vol1['si']>1.15]
        ax2.plot(df_vol1['iNum'],df_vol1[['vol']]*1.05,'r*') #draw marks
        # make these tick labels invisible
        ax2.set_ylabel('vol')
        ax2.grid(True)
        plt.setp(ax2.get_xticklabels(), visible=True)
...
#    pro = ts.pro_api()
#    df = ts.pro_bar(api=pro, ts_code='000001.SZ', adj='qfq',start_date='20181201')
    #df = ts.pro_bar(ts_code='399005.SH', asset='I', start_date='20180101', end_date='20181212')
    #df=df.sort_values('trade_date')
    
    df=fd.YFetchData('600030','2018-01-01').data
