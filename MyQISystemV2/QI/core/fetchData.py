...
class TFetchData(FetchData):
    ...
    def _get_data(self):
        try:
            ...
            df.index=pd.DatetimeIndex(td1['trade_date'])
            df.loc[:,'ts_code']=self.symbol
        except:
            pass
        return df
...
if __name__=='__main__': 
    td1=YFetchData('600600','2018-01-01').data
    td1.loc[:,'high_rate']=(td1['high']-td1['open'])/td1['open']*100
    td1.loc[:,'low_rate']=(td1['low']-td1['open'])/td1['open']*100
    td1.loc[:,'ch_rate']=(td1['high']-td1['low'])/td1['open']*100
    
    td1_lowout=td1[td1['low_rate']<-3]
    td1_highout=td1[td1['high_rate']>5]
    td1_ch_max=td1[td1['ch_rate']>8]
    
    
    print(len(td1[td1['high_rate']>4.0])/len(td1))
    print(len(td1[td1['low_rate']<-4.0])/len(td1))
    print(len(td1[td1['ch_rate']>8])/len(td1))
    #high_mean_value=td1.loc[:,'high_rate'].mean()
   # high_std_value=td1.loc[:,'high_rate'].std()
    

    #td2=YFetchData('600004','2018-01-01').data
